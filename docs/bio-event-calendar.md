...
The root `index.html`, `app.js`, and `data/bio_events.json` files are the GitHub Pages site. The `Deploy GitHub Pages` workflow builds a minimal `_site` artifact from those files and deploys it on pushes to `main`/`master` or manual dispatch.
